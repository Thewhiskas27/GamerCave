...
public class ConsoleUI
{
    ...
    public virtual IPlayer Player(string name) { return players[name]; }

    public virtual void SetPlayer(string name, IPlayer player) { players[name] = player; }
    ...
    public virtual void SelectPlayers(Game game)
    {
        ...
        SetPlayer(choice, new Human(game, input, output));
        SetPlayer(computer, new CPU(game));
    }
    ...
    {
        if (game.IsDraw()) output.WriteLine("Draw!");
        else output.WriteLine("Player {0} Wins!", game.Winner);
    }
}